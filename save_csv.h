...
/** @brief Saves a data table in as CSV for spreadsheet software.
 *  
 ...
 */
bool save_csv(std::ostream &out, std::map<std::string, std::vector<double> > data);
 ...
 *  @returns : true on error, false if all is well.
 */
inline bool save_csv(std::string filename, std::map<std::string, std::vector<double> > data) {
	using namespace std;
	ofstream out(filename.c_str(), ios::out|ios::trunc);
	return save_csv(out, data);
}
